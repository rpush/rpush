module Rpush
  module Upgraded
    def self.check(options = {})
      ...
      begin
        count = Rpush::App.count
      rescue ActiveRecord::StatementInvalid
        puts "!!!! RPUSH NOT STARTED !!!!"
        puts
      ...
      if count == 0
        Rpush.logger.warn("You have not created an app yet. See https://github.com/ileitch/rapns for instructions.")
      end

      if File.exists?(File.join(Rails.root, 'config', 'rapns', 'rapns.yml'))
        Rpush.logger.warn(<<-EOS)
