...
describe Rpush::Daemon, "when starting" do
  ...
  it 'releases the store connection' do
    described_class.store = double
    expect(described_class.store).to receive(:release_connection)
    described_class.start
  end
  ...
  it 'instantiates the store' do
    ...
    described_class.start
    expect(described_class.store).to be_a(Rpush::Daemon::Store::ActiveRecord)
  end
  ...
  it 'logs an error if the store cannot be loaded' do
    ...
    allow(described_class).to receive(:exit) { described_class.store = double.as_null_object }
    described_class.start
  end

  it "writes the process ID to the PID file" do
    expect(described_class).to receive(:write_pid_file)
    described_class.start
  end
