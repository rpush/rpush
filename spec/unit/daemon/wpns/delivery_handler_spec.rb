...
describe Rapns::Daemon::Wpns::DeliveryHandler do
  ...
  let(:queue) { Queue.new }
  let(:app) { double }
  let(:delivery_handler) { Rapns::Daemon::Wpns::DeliveryHandler.new(app) }
  let(:http) { double(:shutdown => nil) }
  let(:delivery) { double(:perform => nil) }

  before do
    Net::HTTP::Persistent.stub(:new => http)
    Rapns::Daemon::Wpns::Delivery.stub(:new => delivery)
  ...
  it 'performs delivery of an notification' do
    Rapns::Daemon::Wpns::Delivery.should_receive(:new).with(app, http, notification, batch).and_return(delivery)
  ...
  it 'initiates a persistent connection object' do
    ...
    Rapns::Daemon::Wpns::DeliveryHandler.new(app)
  end
